...
同步成功后，部署工作流通过 `workflow_run` 自动构建并发布 main 的最新数据。
不要只依赖数据提交触发 `push`：使用 `GITHUB_TOKEN` 创建的提交不会触发另一个 push 工作流。

### 学期切换与数据时效

2026-09-14 核查：学校已进入 26-27 第1学期，但两类公共教室查询页仍返回
25-26 第2学期。同步时间仅表示抓取时间，不能证明课表已更新。
前端会默认隐藏旧学期查询结果，允许用户主动展开历史课表。
每学期按学校通知更新 `web/src/semester.ts` 中的 `REQUIRED_SEMESTER`；
公开源提供该学期或后续学期后，下一次同步及部署会自动恢复查询。
不要直接修改 `data/classrooms.json` 的学期名称来冒充新学期课表。

学期保护逻辑测试（Node.js 22.18+）：`cd web && node --test tests/semester.test.mjs`。
